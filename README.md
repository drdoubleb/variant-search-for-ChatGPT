...
- **Per-request caps (always on):** the completion is capped at `max_tokens` 3000, the context is clamped to 1,500,000 chars, user notes to 4,000 chars, and request bodies over ~4 MB (just under Vercel's serverless request-body ceiling) are rejected with `413`. These bound the cost of any single call. The context clamp is generous because rich genes assemble very large payloads (e.g. HER2/ERBB2 amplification ≈ 957 KB / ~219k tokens); note that such contexts may exceed some models' context windows and be costly — prefer a large-window model, or trim the payload.
